...
15. Go back to the pipeline list page. You should you your new pipeline at the top named `shotgunsoftware.<repo-name>`. We'll need to rename it to just the repository name (this will help our release scripts.). We'll also want to move it to one of the appropriate folder in the `All` tab (nudged between `Recent` and `Runs`). To do this, click on the pipeline. Once you're on the page of the pipeline, click on the `...` button and select `Rename/Move`. Remove the organization name, select the right after and hit `Save`.
